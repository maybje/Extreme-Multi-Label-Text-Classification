...
if file=="econbiz":
    x=tfidf_matrix(file,folds,t,2)  #X matrix
    ...
    x_t,x_v=tfidf_matrix_med(file,folds,t,2)     #X matrix
...
#defining model for Econbiz dataset
def create_model_eco():
    model = models.Sequential() #defining sequential model
    model.add(Dense(1500, activation='relu', input_shape=(nor,))) #hidden layer
...
#call modelfunction depending on the file
if file=="econbiz":
    model=create_model_eco()
else:
    model=create_model_med()
#fitting model
...
#results=results_path+"\\mlp_t"+str(t)+"_"+file+".txt"
#res = open(results,'w')
