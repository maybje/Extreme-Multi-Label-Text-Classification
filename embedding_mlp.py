...
def create_model_eco():
    ...
    model.add(Dense(1500, activation='relu'))   #hidden fully connected layer
...
def create_model_med():
    ...
    model.add(Embedding(len(word_index) + 1,
                                ...
                                trainable=False))
    model.add(Flatten())        #flattening output
    model.add(Dense(1000, activation='relu'))    #hidden fully connected layer
    model.add(Dropout(0.5)) #dropout layer
